...
GOBUILD_OPTS=-trimpath -buildmode=pie -mod 'readonly' -modcacherw
...
doc: # Builds the manpage.
	$(SCDOC) <doc/allalt.1.scd >allalt.1

install: # Installs the release binary.
	$(INSTALL) -d \
		$(DESTDIR)$(PREFIX)/$(BINDIR)/ \
		$(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
	$(INSTALL) -pm 0755 allalt $(DESTDIR)$(PREFIX)/$(BINDIR)/
	$(INSTALL) -pm 0644 allalt.1 $(DESTDIR)$(PREFIX)/$(MANDIR)/man1/
